...
def render_structured_data(page: dict, canonical: str) -> str:
    ...
    items = [(BRAND, BASE_URL.rstrip("/") + GURI)]
    for label, href in crumbs:
        url = (BASE_URL.rstrip("/") + href) if href else canonical
        items.append((label, url))
    if not crumbs:
        # 메인 페이지는 자기 자신만
        items = [(BRAND, canonical)]
    bc_items = ",".join(
        f'{{"@type":"ListItem","position":{i+1},"name":"{_json_escape(name)}","item":"{url}"}}'
...
def render_page(page: dict) -> str:
    ...
    structured_data = render_structured_data(page, canonical)
...
{structured_data}{extra_head}</head>
<body>
<header class="site-header">
  <div class="header-accent" aria-hidden="true"></div>
  <div class="header-top">
    <div class="header-inner">
      <a class="brand" href="{GURI}"><span class="brand-mark">바</span> <span class="brand-text">{BRAND}</span></a>
      <p class="header-tagline"><span class="tag-gem">◆</span> 경기도 구리시 전지역 방문 관리 <span class="tag-gem">◆</span> 24시간 상담</p>
  ...
  <div class="container footer-grid">
    <div class="footer-col footer-about">
      ...
      <div class="footer-inquiry">
        <a class="footer-cta" href="{TELEGRAM_BUILD}" target="_blank" rel="noopener nofollow"><span class="tg-ico" aria-hidden="true">✈</span> 웹사이트 제작문의</a>
        <a class="footer-cta" href="{TELEGRAM_PARTNER}" target="_blank" rel="noopener nofollow"><span class="tg-ico" aria-hidden="true">✈</span> 제휴문의</a>
      </div>
    </div>
...
def build() -> None:
    report = []
    indexed = []  # 색인 허용 페이지 (sitemap·RSS 공용)
    ...
    for page in PAGES:
        path = page["path"]  # "" 또는 "galmae-dong/" 형태
    ...
    # sitemap.xml (lastmod 포함 — 색인 갱신 속도 ↑)
    urls = "\n".join(
        f"  <url><loc>{p['url']}</loc><lastmod>{lastmod}</lastmod>"
        f"<changefreq>weekly</changefreq>"
        f"<priority>{'1.0' if p['url'].rstrip('/') == base else '0.7'}</priority></url>"
        for p in indexed
    )
    ...
    # feed.xml (RSS 2.0 — 네이버·구글·빙 피드 수집 및 신규 페이지 발견용)
    items = "\n".join(
        "    <item>\n"
    ...
    with open(os.path.join(ROOT, "feed.xml"), "w", encoding="utf-8") as f:
        f.write(
            ...
            "  <channel>\n"
            f"    <title>{html.escape(BRAND)} · 구리시 출장마사지·홈타이 안내</title>\n"
            f"    <link>{base}/</link>\n"
            f"    <atom:link href=\"{base}/feed.xml\" rel=\"self\" type=\"application/rss+xml\"/>\n"
            "    <description>경기도 구리시 전지역 방문 관리(출장마사지·홈타이) 지역별 안내</description>\n"
    ...
    # IndexNow 키 파일 — 빙·네이버·얀덱스 소유 확인용
